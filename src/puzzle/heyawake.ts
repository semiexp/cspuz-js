...
export function solveHeyawake(height: number, width: number, regions: {y: number, x: number}[][], clues: number[],
                              horizontal: boolean[][], vertical: boolean[][]): number[][] | null {
    let solver = new Solver();
    let isBlack = solver.boolArray([height, width]);
    solver.addAnswerKey(isBlack);
    ...
    for (let y = 0; y < height; ++y) {
        for (let x = 0; x < width; ++x) {
            if (x < width - 1 && vertical[y][x]) {
                let x2 = x + 1;
                while (x2 + 1 < width && !vertical[y][x2]) ++x2;
                if (x2 + 1 < width) {
                    let cells = [];
                    for (let i = x; i <= x2 + 1; ++i) {
                        cells.push(isBlack.at(y, i));
            ...
            if (y < height - 1 && horizontal[y][x]) {
                let y2 = y + 1;
                while (y2 + 1 < height && !horizontal[y2][x]) ++y2;
                if (y2 + 1 < height) {
                    let cells = [];
                    for (let i = y; i <= y2 + 1; ++i) {
                        cells.push(isBlack.at(i, x));
